...
Incremental reconstruction approaches address these issues. Simultaneous Localization and Mapping (SLAM)-methods \cite{cadena_past_2016}  estimate camera poses and scene structure on-the-fly, enabling online operation and continuous map expansion as new images arrive. More recently, learning-based scene representations such as 3DGS have been adapted to online image acquisition, like for example On-the-fly NVS~\cite{meuleman_--fly_2025}. These methods make it possible to visualize a scene in real-time while it is being reconstructed, providing immediate feedback and enabling adaptive data acquisition.
...
The mapping from an arbitrary pose parameterization $\Vector{e}$ (e.g. Euler angles) to its associated homogeneous transformation matrix is denoted by $\Matrix{E}=\Matrix{T}(\Vector{e})$.
...
To enable camera pose tracking with a Mocap system, we attach a camera rigidly to a fixed set of Mocap markers, which is depicted in Figure~\ref{fig:rigid_body}. These markers define a rigid body that can be tracked by the Mocap system. To maintain the analogy to robotics, in the following, we refer to the rigid body as \emph{tool}. The pose of this tool coordinate system (TCS) relative to the Mocap base coordinate system (BCS) can continously be tracked by the Mocap system. It differs from the pose of the camera coordinate system (CCS) relative to the BCS, that we are interested in, by an unknown pose $\TMatrix{C}{T}{T}$, which we call \emph{hand-eye pose}.

\begin{figure}[]
    \centering
    \includegraphics[width=\linewidth]{figures/rigid_body.png}
    \caption{Hand-held rigid body connecting the camera to a spatially fixed set of Mocap markers.}
...
For estimating initial parameters, we use the method of Zhang~\cite{zhang_flexible_2000}. Zhang models the projection of a 3D point in the WCS onto the 2D Image Coordinate System (ICS) through the pinhole camera. This allows for a linear model formulation using homogenous coordinates. The model parameters, namely the intrinsic parameters of the pinhole camera $\Vector{i} = \left(f_x, f_y, c_x, c_y\right)$ and the extrinsic parameters of each image $\Vector{e}_i$, can then linearly be determined.
...
\begin{equation}
    ...
    d_{\mathrm{CC}}(\boldsymbol{\theta}) = \sum_{k=1}^{n_o} \sum_{j=1}^{n_m}
        \left\| \Vector{m}_{j,k} - \boldsymbol{\pi}\!\left( \Vector{M}_j, \boldsymbol{\theta} \right) \right\|_2
...
During the acquisition of the calibration dataset, we not only capture calibration images but also record the corresponding tool pose $\TMatrix{T}{T}{B}$ from the Mocap system for each image, synchronized via $\Delta t$. This allows us to perform a hand-eye calibration.

Hand-eye calibration refers to the determination of the relative pose between a robot's end effector (tool) and a camera that is rigidly mounted to it~\cite{daniilidis_hand-eye_1999}. Establishing this relationship allows observations made in the CCS to be transformed into the TCS and subsequently into the BCS.

In our case, we want to use hand-eye calbration to estimate the relative pose bewtween the CCS and the TCS, which can be tracked with the Mocap system. This hand-eye pose $\TMatrix{C}{T}{T}$ then allows the tool pose $\TMatrix{T}{T}{B}$ to be corrected, retrieving the camera pose $\TMatrix{C}{T}{B}$ which can then be used for the online 3D reconstruction. Figure~\ref{fig:transformations} illustrates the relevant coordinate systems and transformations in this context. As for the camera calibration, our hand-eye calibration procedure is based on OpenCV and consists of an initial parameter estimation followed by a nonlinear refinement.
...
After completing the online 3DGS-based 3D reconstruction, we perform an offline surface extraction step to recover the explicit geometry of the observed scene or object. For this purpose, we adopt the efficient mesh extraction pipeline proposed in SuGaR~\cite{guedon_sugar_2024}, which is straightforward to integrate with a previously trained 3DGS representation.
...
\paragraph{Buddha.}
This dataset consists of 411 RGB images capturing a Buddha statue from multiple concentric viewpoints, each with its corresponding Mocap pose. The object exhibits both coarse geometric structure and fine-grained surface details, rendering it suitable for quantitative 3D evaluation. A highly accurate 3D ground truth of the statue is available in the form of a dense mesh acquired using a structured-light scanning system, for details refer to \cite{petrovska_geometric_2023, petrovska_vision_2024}.
...
\paragraph{PhotoLab.} 
This dataset consists of 176 RGB images capturing the room in which the Mocap system is installed, each with an corresponding Mocap pose. Unlike the previous datasets, the scene is captured in an eccentric, inside-out manner: instead of observing a single object from the outside, the camera traverses the room along a looped trajectory, capturing the surrounding environment. This dataset is particularly challenging, especially for image-based pose estimation methods, as many visual features are only observed from a limited number of viewpoints. A 3D ground truth of the room is not available.
...
MocapGS uses images and Mocap poses as input, while \\COLMAP + 3DGS and On-the-fly NVS only use the captured images as input and infer the pose solely from image data. COLMAP + 3DGS and On-the-fly NVS therefore operate with an unknown scale factor, while MocapGS is able to operate in metric units due to the calibrated Mocap system.
...
\begin{figure*}
	\centering
	\includegraphics[width=\textwidth]{figures/buddha_montage_small.png} \\
    \vspace{0.1cm}
    \includegraphics[width=\textwidth]{figures/robot_montage_small.png} \\
    \vspace{0.1cm}
    \includegraphics[width=\textwidth]{figures/photolab_montage_small.png}
    ...
    \begin{minipage}[t]{0.16\linewidth}
        ...
        MocapGS-RotOpt
    \end{minipage}
    \caption{Test image that has not been used during training (left) and rendered scene for all corresponding methods. Buddha dataset on top, Robot dataset in the middle and PhotoLab dataset at the bottom.}
...
\begin{figure*}
	\centering
	\includegraphics[width=\textwidth]{figures/buddha_montage_3d_small.png} \\
    \vspace{0.1cm}
    \includegraphics[width=\textwidth]{figures/robot_montage_3d_small.png}
...
\begin{figure*}[t]
    \captionsetup[subfigure]{
        labelformat=empty,
        font={sf,small} % use small if your main captions are small
    }
    \centering

    \begin{subfigure}{0.28\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/scene_mocapgs.png}
        \caption{MocapGS-Fixed}
    \end{subfigure}\hfill
    \begin{subfigure}{0.28\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/scene_3dgs.png}
        \caption{3DGS}
    \end{subfigure}\hfill
    \begin{subfigure}{0.28\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/scene_onthefly_nvs.png}
...
MocapGS introduces additional overhead compared to the baseline On-the-fly NVS approach. The required synchronization and calibration procedures take approximately five minutes, representing only a modest time investment. However, the financial cost of the Mocap system is significant in comparison to the baseline setup, which requires only a standard camera.

Also, a limitation of the presented approach lies in the separation between reconstruction and surface extraction. While 3D meshes can be obtained in a post-processing step, the 3DGS representation itself is not explicitly optimized for surface consistency.
